...
class ManifestBuilder {
    constructor(baseDefinition, options) {
        ...
        if (missingFields.length) {
            ...
            throw new error_1.ManifestBuilderError({ cause });
        }
    ...
    addIngredient(input) {
        ...
        if (!ingredient.hash) {
            throw new error_1.IngredientHashMissingError(ingredient);
        }
    ...
    async addThumbnail(thumbnail) {
        const resourceRef = await (0, hash_1.getResourceReference)(thumbnail, __classPrivateFieldGet(this, _ManifestBuilder_definition, "f").label);
    ...
    static createLabel(vendor) {
        const label = `urn:uuid:${(0, node_crypto_1.randomUUID)()}`;
        if (vendor) {
